...
# %cd /content/drive2/My Drive/ycs25/UGANDA/UGA_2018_UNPS_v02_M_CSV
...
def run(file_paths, output_dir):
  ...
  """
  spec_file = f"{UPLOAD_FOLDER}/EoP__Covariate_Selection_Guide.pdf"
  dataset_file = f"{UPLOAD_FOLDER}/unps_7_bid.pdf"
  ...
  """
  # eventually, this will be set by the user somehow
  ...
  for category in folder_to_questionnaire.keys():  # ex. agriculture vs household
    folder_path = folder_to_questionnaire[category].folder_name
    category_prefix = folder_to_questionnaire[category].prefix
    questionnaire_path = folder_to_questionnaire[category].questionnaire_path
    ...
    if category in section_pdf_page_maps.keys():  # if the question map already exists, use it
      ...
      section_pdf_page_map = map_section_to_page_number(questionnaire_path)
    ...
    for section in section_pdf_page_map: # for each section (eventually this list will depend on phase 1)
      ...
      section_questionnaire_pages = [page.page_content for page in load_pdf(questionnaire_path, section_pdf_page_map[section])]
      print("questionnaire pages:", section_questionnaire_pages)
      ...
      # print("Fields with reasoning", fields_with_reasoning, '\n')
    ...
    print("section_fields_dict", section_fields_dict)
  ...
  cur_question_map_path = f'{category}_question_map.csv'
  ...
  for category in folder_to_questionnaire.keys():
    folder_path = folder_to_questionnaire[category].folder_name
